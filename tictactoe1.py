...
class Game(ABC):
    ...
    def gameloop(self):
        current_player = 0  # will be used to switch between players
...
class SinglePlayerMode(Game):
    ...
    def markers_setup(self) -> Tuple[str, str]:
        '''
        Gets valid marker from user and returns that marker alongside
        the complement of that marker (as computer's marker)
        '''
...
class TwoPlayerMode(Game):
    ...
    def markers_setup(self) -> Tuple[str, str]:
        ...
        the complement of that marker (as player 2's marker)
        '''
...
class Cell:
    ...
    def getCoordinate(self) -> Tuple[int, int]:  # TODO: use for highlighting winning combo
        return self.__coordinate


    def setMarker(self, marker: str) -> None:  # TODO: make marker a PROPERTY?
        self.marker = marker
...
class Board:
    def __init__(self):
        self.grid: List[List[Cell]] = [ [Cell(coordinate=(row, col)) for col in range(BOARD_DIMENSIONS)] for row in range(BOARD_DIMENSIONS) ]
        self.empty_coordinates: Set[int] = {(i, j) for i in range(BOARD_DIMENSIONS) for j in range(BOARD_DIMENSIONS)}


    def update(self, marker: str, coordinate: Tuple[int, int]) -> None:
        '''
    ...
    def winner(self) -> Optional[str]:
        ...
        '''
        board: List[List[Cell]] = self.grid
    ...
    def result(self, current_marker: str, coordinate: Tuple[int, int]) -> Board:
        """
    ...
    @staticmethod
    def position_from_coordinate(coordinate: Tuple[int, int]) -> int:
        row, col = coordinate[0], coordinate[1]
        return (3 * row) + col + 1  # this formula gives for board positions from 1 to 9

    @staticmethod
    def coordinate_from_position(pos: int) -> Tuple[int, int]:
        if 1 <= pos <= 3:
            row = 0
        elif 4 <= pos <= 6:
            row = 1
...
class AI(Player):
    ...
    def make_move(self, board: Board) -> Tuple[int, int]:
        ...
        print(f"-> {self.alias} ({self.marker}) turn")
        print(board)
        print(f"{self.alias} thinking....")

        if board.terminal():
            return None

        if self.marker == X:
            best_utility = Game.X_utility
        else:
           best_utility = Game.O_utility

        if best_utility == 1:  # AI is maximizing player
            return self.get_max_coordinate(board)
        else:  # AI is minimizing player
            return self.get_min_coordinate(board)


    def get_min_coordinate(self, board: Board) -> Tuple[int, int]:
        '''takes a board as input and returns the coordinate
        where making a move would lead to the least possible utility
        on that board'''
        # find the minimum possible utility on the input board state if both players play optimally
        best_utility_sofar = 1
        alpha, beta = -1, 1
        empty_coordinates = board.empty_coordinates
        for coordinate in empty_coordinates:
            best_utility_sofar = min(best_utility_sofar, self.get_max_utility(board.result(self.marker, coordinate), alpha, beta))
        ...
        for coordinate in empty_coordinates:
            if self.get_max_utility(board.result(self.marker, coordinate), alpha, beta) == best_utility_sofar:
                pos = Board.position_from_coordinate(coordinate)
                print(f"Position decided: {pos}")
                return coordinate


    def get_max_coordinate(self, board: Board) -> Tuple[int, int]:
        '''takes a board as input and returns the coordinate
        where making a move would lead to the highest possible utility
        on that board'''
        # find the maximum possible utility on the input board state if both players play optimally
        best_utility_sofar = -1
        alpha, beta = -1, 1
        empty_coordinates = board.empty_coordinates
        for coordinate in empty_coordinates:
            best_utility_sofar = max(best_utility_sofar, self.get_min_utility(board.result(self.marker, coordinate), alpha, beta))
            if best_utility_sofar == 1:
                pos = Board.position_from_coordinate(coordinate)
                print(f"Position decided: {pos}")
                return coordinate
        ...
        # find the coordinate that lead to the best possible utility calculated above
        for coordinate in empty_coordinates:
            if self.get_min_utility(board.result(self.marker, coordinate), alpha, beta) == best_utility_sofar:
                pos = Board.position_from_coordinate(coordinate)
                print(f"Position decided: {pos}")
                return coordinate
